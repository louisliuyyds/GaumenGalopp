# controllers/restaurant_oeffnungszeit_controller.py - KORRIGIERT
...
def get_for_restaurant(restaurantid: int, db: Session = Depends(get_db)):
    service = RestaurantOeffnungszeitService(db)
    items = service.get_by_restaurant_id(restaurantid)
    return items
...
def get_active_for_restaurant(restaurantid: int, db: Session = Depends(get_db)):
    service = RestaurantOeffnungszeitService(db)
    items = service.get_active_by_restaurant_id(restaurantid)
    return items
...
def get_current_for_restaurant(restaurantid: int, db: Session = Depends(get_db)):
    service = RestaurantOeffnungszeitService(db)
    item = service.get_current_for_restaurant(restaurantid)

    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No current opening hours found for restaurant {restaurantid}"
        )
...
def delete_restaurant_oeffnungszeit(
    ...
    service = RestaurantOeffnungszeitService(db)
    success = service.delete(restaurantid, oeffnungszeitid, gueltig_von)
