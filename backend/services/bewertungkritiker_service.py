...
class BewertungkritikerService:
    ...
    def get_all(self) -> List[Bewertungkritiker]:
        # KORRIGIERT: Filter 'is_active' entfernt
        return self.db.query(Bewertungkritiker).all()

    def get_by_id(self, bewertungkritiker_id: int) -> Optional[Bewertungkritiker]:
        # WICHTIG: Ich habe hier '.id' zu '.bewertungkritikerid' geändert.
        # Bitte prüfe kurz in 'models/bewertungkritiker.py', ob der Primary Key so heißt!
        return self.db.query(Bewertungkritiker).filter(Bewertungkritiker.bewertungkritikerid == bewertungkritiker_id).first()
    ...
    def delete(self, bewertungkritiker_id: int) -> bool:
        ...
        self.db.delete(bewertungkritiker)
